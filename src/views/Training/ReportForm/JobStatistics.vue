<template>
  <div class="CareerGrowth-index TrainingReport-job height100">
    <!-- 页面右边按钮***start -->
    <div class="bt-container" style="position:absolute;right:20px;top:128px">
      <el-button type="primary" size="mini">{{$t('Training.TrainingReport.info[4]')}}</el-button>
    </div>
    <div class="right-btn" style="top:74px">
      <span>
        ...
        <span
          style="color:#00A6FF"
        >{{$t('Training.TrainingReport.form2')}}</span>
      </span>
    </div>
...
export default {
  ...
  created() {
    this.getCareer();
    this.PostTotalTrainReport();
  },
  watch: {},
  computed: {},
  methods: {
    PostTotalTrainReport() {
      httpApi
        .PostTotalTrainReport({
          course_type: 0,
    ...
    handCount(row) {
      console.log(row);
      this.$router.push(
        `/Training/ReportForm/TeachersReport?re_career_id=${row.career_id}`
      );
    },
    // 编辑列编辑提交
    handSetting(v) {
    ...
    this.$nextTick(() => {
      if (this.$refs["auto_height_table"]) {
        this.tableList.options.tableHeight = this.$Tool.getTableHeight(
          this.$refs.auto_height_table.getBoundingClientRect().top + 10
        );
      }
    });
  }
