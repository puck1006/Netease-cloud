<template>
  <div class="Educational-circulateRefistrat height100">
    <div class="flex-all pt10 pb10 pl12 pr12 bg-white height50 bottom-line-f2f7">
      <div class="ct-head-left flex-al">
        ...
        <span class="color-243 fs18">{{$t("Educational.circulateRegistrat.title")}}</span>
      </div>
      <div class="cr-head-right fs12">
        <span>{{$t("Common.header.nav_name3")}}</span>>
        <span class="color-blue">{{$t("Educational.circulateRegistrat.title")}}</span>
...
<script>
export default {
  name: "Educational-circulateRefistrat",
  data() {
    return {
      innerSlider: [
        {
          ...
          module_id: "115",
          postrole_id: null,
          url: `/Educational/circulateRegistrat/classRecord?class_id=${this.$route.query.class_id}&type=${this.$route.query.type}`
        },
        {
          title: "电访管理",
          module_id: "116",
          postrole_id: null,
          url: `/Educational/circulateRegistrat/callManagement?class_id=${this.$route.query.class_id}`
        }
      ]
    };
  },
...
<style lang="less">
.Educational-circulateRefistrat {
}
</style>
