        ...
        <div class="flex-al mb12">
          ...
          <div class="select-box" style="width: 170px">
            <el-select
              ...
              filterable
              :placeholder="
                $t('Educational.educationalReport.classReport.class_ph')
              "
              @change="search"
            >
              <el-option
      ...
      <div class="tableDiv" ref="auto_height_table">
        ...
        >
          ...
          <el-table-column
            ...
            :label="
              $t(
                'views.Educational.EducationalReport.reportBox.classReport.55twjy1phsc0'
              )
            "
            align="center"
            width="150px"
...
export default {
    ...
    return {
      ...
      tableList: {
        ...
        options: {
          ...
          tableHeight: 0, //表格高度
          moduleName: "generalStatistics", //当前页面所属于模块的名称****一个模块是唯一的
          ColumnName: "generalStatistics", //当前页面的名称，通常与路由名称一致
        },
      },
      // 分页
      paging: {
  ...
  created() {
    this.filter.day = this.$Tool.getCurrMonth();
  ...
  methods: {
    handleDetail(row) {
      console.log(row);
      this.$router.push(
        `/Educational/circulateRegistrat/callManagement?class_id=${row.class_id}&day=${this.filter.day}`
      );
    },
    setStorge() {
