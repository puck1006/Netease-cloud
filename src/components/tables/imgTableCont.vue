<template>
<!-- 表格说明： 此表格主要用于表格中存在图片的情况，同时兼具基本表格的功能：复选框，编辑列，表格列中内容有点击事件-->
  <div class="table-content" :style="{'height':(tableData.options.tableHeight+'px')}">
    <el-table
      :data="tableData.list"
      border
      fit
        ...
        <template v-if="$index==IndexId || $index==IndexIdTwo ||$index==0">
          ...
          >
            <template slot-scope="scope">
              <div
                v-if="column.iscodeimg"
                class="table-code"
                @click="handleCodeChange(scope.$index,scope.row,column)"
              >
                <img :src="tableData.list[scope.$index][column.fieldstring]" />
              </div>
              <el-button
