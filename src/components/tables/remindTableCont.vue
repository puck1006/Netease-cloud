<template>
  <!-- 表格说明： 此表格主要为备课提醒表格，有未读标识，兼具基本表格的功能：复选框，编辑列，表格列中内容有点击事件-->
  <div
    class="table-content remindTableCont"
      ...
      <template v-for="(column, $index) in tableData.columns">
        ...
        <template v-if="$index == IndexId || $index == IndexIdTwo || $index == 0">
          ...
          >
            <template slot-scope="scope">
              <!-- 文字标识 -->
              <span v-if="column.judgeStause &&scope.row.hour_read_status=='0'" class="icon-mark">
                {{
                $t("Common.noread_text")
                }}
              </span>
              <el-button
        ...
        <template v-else>
          ...
          >
            <template slot-scope="scope">
              <!-- 文字标识 -->
              <span v-if="column.judgeStause &&scope.row.hour_read_status=='0'" class="icon-mark">未读</span>
              <el-button
                v-if="column.ismethod"
                type="text"
                class="link-type"
...
.remindTableCont {
  ...
  .icon-mark {
    ...
    color: #fff;
    font-size: 12px;
    background: url("../../assets/images/no-read-bg.png") no-repeat;
