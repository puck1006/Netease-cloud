<template>
  <!-- 表格说明： 此表格主要用于表格中存在select选择框的情况***此表格组件针对基本样式的select选择框，同时兼具基本表格的功能：复选框，编辑列，表格列中内容有点击事件-->
  <!-- 适用于教务系统 小循环登记 -- 网课参与 -->
  <div
    class="table-content onlineClassTable"
      ...
      <template v-for="(column, $index) in tableData.columns">
        ...
        <template v-if="$index == IndexId || $index == IndexIdTwo || $index == 0">
          <el-table-column
            ...
            align="center"
            :min-width="column.width ? column.width : '200px'"
          ...
          >
            <template slot-scope="scope">
              ...
              <el-select
                v-else-if="column.isSelect"
                v-model="scope.row[column.fieldstring]"
                :placeholder="
                  $t('Educational.circulateRegistrat.onlineclass.placehold')
                "
                @change="
                  handClickChange(
                    scope.row,
              ...
              >
                <el-option
                  :value="1"
                  :label="
                    $t('Educational.circulateRegistrat.onlineclass.select[0]')
                  "
                ></el-option>
                <!-- <el-option
                  :value="0"
                  :label="$t('Educational.circulateRegistrat.onlineclass.select[2]')"
                ></el-option>-->
                <el-option
                  :value="-1"
                  :label="
                    $t('Educational.circulateRegistrat.onlineclass.select[1]')
                  "
                ></el-option>
              </el-select>
              <el-input
                v-else-if="scope.row.hour_netstatus == 1 && column.isInputText"
                v-model="tableData.list[scope.$index][column.fieldstring]"
                :placeholder="column.palchoderTex"
                @input="
                  handleInput(
                    scope.row,
                    scope.$index,
                ...
                :disabled="column.disabled"
              ></el-input>
              <span v-else-if="scope.row.hour_netstatus != 1 && column.isInputText" type="text">--</span>
        ...
        <template v-else>
          <el-table-column
            ...
            :sortable="column.sortable"
            align="center"
            :min-width="column.width ? column.width : '275px'"
            :show-overflow-tooltip="true"
            v-if="column.show == '1'"
          >
            <!-- 自定义表头 -->
            <template slot="header" slot-scope="scope">
              <span v-if="column.showSelect" class="flex-all" style="justify-content: center;">
                {{ column.fieldname }}:
                <el-select
                  v-model="select"
                  :placeholder="$t('Educational.circulateRegistrat.onlineclass.placehold')"
                  @change="handSelect(select,scope)"
                  clearable
                  style="width:190px"
                >
                  <el-option
              ...
              <span v-else>{{ column.fieldname }}</span>
            </template>
            <template slot-scope="scope">
              <el-button
                v-if="column.ismethod"
...
export default {
  ...
  methods: {
    ...
    //input输入 - change事件
    handleInput(v, index, value) {
      // console.log(v, index, value);
      this.$emit("handleInput", v, index, value);
    },
    handClickChange(row, index, v, c) {
      this.$emit("handClickChange", row, index, v, c);
    },
    // input输入 - blur事件
    handleBlurInput(v, index, value, c) {
      this.$emit("handleBlurInput", v, index, value, c);
    },
    handSelect(select, scope) {
      this.$emit("handSelect", select, scope);
    }
  }
};
</script>
