<template>
  <!-- 表格说明： 此表格主要用于表格中有进度条显示的情况，同时兼具基本表格的功能：复选框，编辑列，表格列中内容有点击事件-->
  <div
    class="table-content progressTableCont"
  ...
  >
    <el-table
      ...
      :header-cell-style="{background:'#F9FCFF'}"
      style="width: 100%"
    >
      <!-- 暂无数据提示，自己定义就OK了 -->
      <template slot="empty">
      ...
      <template v-for="(column,$index) in tableData.columns">
        ...
        <template v-if="$index==IndexId || $index==IndexIdTwo ||$index==0">
          <el-table-column
            ...
            align="center"
            :min-width="column.isProgres?'200px':'150px'"
            v-if="column.show=='1'"
          >
            <template slot-scope="scope">
              <!-- 判断进度条 -->
              <div v-if="column.isProgress" class="progress table-progress flex-al ml10">
                <div class="out-progress">
                  <div
                    class="inner-progress"
                    :style="{width: parseInt(scope.row[column.fieldstring].split('/')[0])>parseInt(scope.row[column.fieldstring].split('/')[1])?scope.row[column.fieldstring].split('/')[1]:(scope.row[column.fieldstring].split('/')[0] / scope.row[column.fieldstring].split('/')[1]) * 100 + '%' }"
                  ></div>
                </div>
                <span
                  v-if="parseInt(scope.row[column.fieldstring].split('/')[0])==parseInt(scope.row[column.fieldstring].split('/')[1]) && parseInt(scope.row[column.fieldstring].split('/')[1]) != 0"
                >已完成</span>
                <span
                  v-if="parseInt(scope.row[column.fieldstring].split('/')[0])<parseInt(scope.row[column.fieldstring].split('/')[1]) && parseInt(scope.row[column.fieldstring].split('/')[0]) != 0"
                >进行中</span>
                <span
                  v-if="parseInt(scope.row[column.fieldstring].split('/')[0]) == 0 && parseInt(scope.row[column.fieldstring].split('/')[1]) != 0"
                >待完成</span>
                <span class="ml5">{{tableData.list[scope.$index][column.fieldstring]}}</span>
              </div>
              <el-button
        ...
        <template v-else>
          <el-table-column
            ...
            align="center"
            :show-overflow-tooltip="true"
            :min-width="column.isProgres?'200px':'190px'"
            v-if="column.show=='1'"
          >
            <template slot-scope="scope">
             
              <!-- 判断进度条 -->
              <div v-if="column.isProgress" class="progress table-progress flex-al">
                <div class="out-progress">
                  <div
                    class="inner-progress"
                    :class="{innerWidth:parseInt(scope.row[column.fieldstring].split('/')[1]) == 0}"
                    :style="{width: parseInt(scope.row[column.fieldstring].split('/')[0])>parseInt(scope.row[column.fieldstring].split('/')[1])?scope.row[column.fieldstring].split('/')[1]:(scope.row[column.fieldstring].split('/')[0] / scope.row[column.fieldstring].split('/')[1]) * 100 + '%' }"
                  ></div>
                </div>
                <!-- <span
                ...
                <span
                  v-if="parseInt(scope.row[column.fieldstring].split('/')[0]) == 0 && parseInt(scope.row[column.fieldstring].split('/')[1]) != 0"
                >待完成</span> -->
                <span class="ml5">{{scope.row[column.fieldstring]}}</span>
              </div>
              <el-button
                v-else-if="column.ismethod"
                type="text"
