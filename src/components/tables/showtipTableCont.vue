<template>
  ...
  <div
    class="table-content showtipTableCont"
        ...
        <template v-else>
          ...
          >
            ...
            <template slot="header" slot-scope="scope">
              <span v-if="column.isTip">
                {{ column.fieldname }}
                <el-popover
                  trigger="click"
                  placement="bottom"
                  class="explanation-button"
                  popper-class="explanation-button-tip"
                >
                  <div class="info-body">
                    <div class="top pl12 pr12 fs18 color-333">{{ tips }}</div>
                  </div>
                  <span slot="reference" class="explanation-wrapper">
                    <el-button type="text">
                      <i class="el-icon-question mr5"></i>
                    </el-button>
                  </span>
                </el-popover>
              </span>
...
export default {
  ...
  data() {
    return {
      ...
      IndexId: -1, //固定的列数
      IndexIdTwo: -1, //固定的列数
      tips: "根据班级排课日期计算出截至当前应该上课的次数，比上班级总课次数"
    };
  },
  created() {
    this.getColums();
